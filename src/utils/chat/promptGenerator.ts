...
3. Format your response in markdown. IMPORTANT: All mathematical expressions MUST use dollar-sign delimiters. Use $...$ for inline math (e.g. $\\theta$, $f_{\\theta}(\\cdot)$, $\\mathcal{L}_C$) and $$...$$ for display/block math. Never use parentheses around raw LaTeX like (\\theta) — always use $\\theta$ instead. Never use \\(...\\) or \\[...\\] delimiters.
...
2. Format your response in markdown. IMPORTANT: All mathematical expressions MUST use dollar-sign delimiters. Use $...$ for inline math (e.g. $\\theta$, $f_{\\theta}(\\cdot)$, $\\mathcal{L}_C$) and $$...$$ for display/block math. Never use parentheses around raw LaTeX like (\\theta) — always use $\\theta$ instead. Never use \\(...\\) or \\[...\\] delimiters.
